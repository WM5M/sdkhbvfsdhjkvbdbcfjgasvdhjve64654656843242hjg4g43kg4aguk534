...
<head>
    ...
    <style>
        * {
        ...
        /* Compact menu container - NOT full-screen */
        #menu-container {
            position: absolute;
            top: 20px;
            left: 20px;
            ...
            background-color: rgba(0, 0, 0, 0.9); /* fallback dark overlay */
        ...
        /* Keyboard Input - starts hidden, no auto-show */
#keyboardContainer {
    display: none !important; /* Force hide on load - no double prompt */
    position: absolute;
    top: 50%;
    left: 50%;
...
<body>
    ...
    <script>
        ...
        window.addEventListener('message', function(event) {
            ...
            // Update elements list with proper On/Off spacing
...
if (data.action === 'updateKeyboard') {
    ...
    if (data.visible) {
        document.getElementById('keyboardTitle').innerText = data.title || 'Choose Menu Key';
        document.getElementById('keyboardInput').value = data.value || '';
        kb.focus(); // Optional: focus the input for better UX
    }
}

            // Arrow navigation support (from Lua keydown messages)
