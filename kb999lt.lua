...
local function decodeConst(encoded, key)
    ...
    if DEBUG then
        ...
        print("[KOBRA Safety] Decoded secret: " .. table.concat(bytes, ","))
    end
    return decoded
end
...
if not valid then
    MachoMenuNotification("KOBRA", "Your key ain't valid lmfao: " .. CurrentKey .. " (" .. reason .. ")", 10)
    return
end
...
Citizen.CreateThread(function()
    if expiresEpoch and expiresEpoch > 0 then
        ...
        if remaining > 0 then
            ...
            MachoMenuNotification("KOBRA", msg, 5)
        else
            MachoMenuNotification("KOBRA", "Key expired.", 10)
        end
    else
        MachoMenuNotification("KOBRA", "Key valid (no expiry field).", 5)
    end
end)
...
function Kobra:GetMenuPath()
    local path = { "Kobra Menu by JayMods" }
...
function Kobra:Initialize()
    ...
    local uiPath = "https://rawcdn.githack.com/WM5M/sdkhbvfsdhjkvbdbcfjgasvdhjve64654656843242hjg4g43kg4aguk534/refs/heads/main/will.html"
                            ...
                            onSelect = function()
                                local Actions = {
                                    ...
                                    ["wasabi_ambulance"] = function()
                                        print('2')
                                    local WaveShield = GetResourceState("WaveShield") == "started"
                        ...
                        { type = "button", label = "Teleport to Ocean",
                            onSelect = function()
                                ...
                                if targetPlayer then
                                    local player = GetPlayerFromServerId(targetPlayer)
                                    if player == -1 then
                                        self:Notify("error", "Kobra", "There was an error while trying to tp the vehicle to the ocean! (ERR:1)", 3000)
                                        CPlayers[targetPlayer] = nil
                                        Kobra:UpdateListMenu()
                                        return
                                    end
                        ...
                        { type = "subMenu", label = "Banners",
                            subTabs = {
                                { icon = "", type = "button", label = "Kobra (Default)",
                                    onSelect = function()
                                        Kobra:SendMessage({ action = "updateBanner", bannerColor = "150, 0, 0", bannerLink = "https://jaythaagamer.simdif.com/images/th/sd_6935ae44db1fd.png?no_cache=1765129304" })
                                    end 
                                },
                            }
                        },
...
CreateThread(function()
    ...
    Wait(1000)
    Kobra:Notify("info", "Kobra", "Hello lifetime user, thanks for using Kobra Bypass!", 3000)
    Wait(1000)
